...
Public Class Tambah
    ...
    Sub tampilkanPanel(ByVal namaForm As Form)
        ...
        Admin.panelUtama.Controls.Add(namaForm)
        namaForm.Show()
    End Sub
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        ...
        Dim gambarProduk As String = Path.GetFileName(opnfileGambar.FileName)

        'kode nama stok harga gambar
        Dim query As String = "Insert Into produk(kode, nama, stok, harga, gambar) Values (NULL, '" & namaProduk & "', '" & stokProduk & "', '" & hargaProduk & "', '" & gambarProduk & "')"
        CMD = New MySqlCommand(query, CONN)
        CMD.ExecuteNonQuery()
        MsgBox("Produk Berhasil Ditambahkan", MsgBoxStyle.Information, "Pemberitahuan")
        bersihkan()
    ...
    Private Sub Tambah_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        koneksi()
        Admin.txtCari.Visible = False
        Admin.btnCari.Visible = False
        Admin.btnTambah.Visible = False
        Admin.btnRefresh.Visible = False
        bersihkan()
    End Sub
End Class